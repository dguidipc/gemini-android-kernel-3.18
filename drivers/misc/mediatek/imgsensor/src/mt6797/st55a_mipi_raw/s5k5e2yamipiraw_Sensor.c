...
//#define S5K5E2YA_OTP_FUNCTION
kal_uint16 module_id_new = 0;
kal_uint16 module_id_old = 0;
...
static imgsensor_info_struct imgsensor_info = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb, // Gb --> Gr
	.mclk = 24,
	.mipi_lane_num = SENSOR_MIPI_2_LANE,
	.i2c_addr_table = {0x20 , 0xff},
    /* cm-baoxuegui, 20160720, add for nice cam + */
...
void otp_read_old_id(void)
{
	kal_uint16 lens_id;
	write_cmos_sensor(0x0a00,0x04);
	write_cmos_sensor(0x0a02,0x03);
	write_cmos_sensor(0x0a00,0x01);
	mdelay(5);
	module_id_old = read_cmos_sensor(0x0a04);
	lens_id = read_cmos_sensor(0x0a05);
	write_cmos_sensor(0x0a00,0x04);
	write_cmos_sensor(0x0a00,0x00);
	mdelay(5);
	LOG_INF("module_id_old = 0x%x\n",module_id_old);
}
#ifdef S5K5E2YA_OTP_FUNCTION
kal_uint32 OFILM_tRG_Ratio_typical = 0x2e8;//0x2D1;
kal_uint32 OFILM_tBG_Ratio_typical = 0x260;//0x274;
...
void S5K5E2YA_OFILM_MIPI_read_otp_wb(struct S5K5E2YA_OFILM_MIPI_otp_struct *otp)
{
	...
	write_cmos_sensor(0x0a00,0x01);
	mdelay(5);
	flag1 = read_cmos_sensor(0x0a05);
	flag2 = read_cmos_sensor(0x0a15);
	flag3 = read_cmos_sensor(0x0a25);
	
	if(0x01 == flag1){
		otp->Module_ID= read_cmos_sensor(0x0a06);
		otp->lens_id = read_cmos_sensor(0x0a07);
		otp->year = read_cmos_sensor(0x0a08);
		otp->mouth = read_cmos_sensor(0x0a09);
		otp->day = read_cmos_sensor(0x0a0a);
		otp->rg_h = read_cmos_sensor(0x0a0b);
		otp->rg_l = read_cmos_sensor(0x0a0c);
		otp->bg_h = read_cmos_sensor(0x0a0d);
		otp->bg_l = read_cmos_sensor(0x0a0e);
		otp->GbGr_h= read_cmos_sensor(0x0a0f);
		otp->GbGr_l = read_cmos_sensor(0x0a10);
		module_id_new = otp->Module_ID;
		LOG_INF("[S5K5E2YA_OFILM_MIPI_read_otp_wb1]%x,%x,%x,%x,%x,%x,%x,%x\n",module_id_new,otp->lens_id
			,otp->rg_h,otp->rg_l,
			otp->bg_h,otp->bg_l,otp->GbGr_h,otp->GbGr_l);	
		mdelay(3);
		write_cmos_sensor(0x0a00,0x04);
		write_cmos_sensor(0x0a00,0x00);
		mdelay(5);
	}else if(0x01 == flag2){				
		otp->Module_ID= read_cmos_sensor(0x0a16);
		otp->lens_id = read_cmos_sensor(0x0a17);
		otp->year = read_cmos_sensor(0x0a18);
		otp->mouth = read_cmos_sensor(0x0a19);
		otp->day = read_cmos_sensor(0x0a1a);
		otp->rg_h = read_cmos_sensor(0x0a1b);
		otp->rg_l = read_cmos_sensor(0x0a1c);
		otp->bg_h = read_cmos_sensor(0x0a1d);
		otp->bg_l = read_cmos_sensor(0x0a1e);
		otp->GbGr_h= read_cmos_sensor(0x0a1f);
		otp->GbGr_l = read_cmos_sensor(0x0a20);
		module_id_new = otp->Module_ID;
		LOG_INF("[S5K5E2YA_OFILM_MIPI_read_otp_wb2]%x,%x,%x,%x,%x,%x,%x,%x\n",module_id_new,otp->lens_id
			,otp->rg_h,otp->rg_l,
			otp->bg_h,otp->bg_l,otp->GbGr_h,otp->GbGr_l);	
		mdelay(5);
		write_cmos_sensor(0x0a00,0x04);
		...
		otp->Module_ID= read_cmos_sensor(0x0a26);
		otp->lens_id = read_cmos_sensor(0x0a27);
		otp->year = read_cmos_sensor(0x0a28);
		otp->mouth = read_cmos_sensor(0x0a29);
		otp->day = read_cmos_sensor(0x0a2a);
		otp->rg_h = read_cmos_sensor(0x0a2b);
		otp->rg_l = read_cmos_sensor(0x0a2c);
		otp->bg_h = read_cmos_sensor(0x0a2d);
		otp->bg_l = read_cmos_sensor(0x0a2e);
		otp->GbGr_h= read_cmos_sensor(0x0a2f);
		otp->GbGr_l = read_cmos_sensor(0x0a30);
		module_id_new = otp->Module_ID;
		LOG_INF("[S5K5E2YA_OFILM_MIPI_read_otp_wb3]%x,%x,%x,%x,%x,%x,%x,%x\n",module_id_new,otp->lens_id
			,otp->rg_h,otp->rg_l,
			otp->bg_h,otp->bg_l,otp->GbGr_h,otp->GbGr_l);	
		mdelay(5);
		write_cmos_sensor(0x0a00,0x04);
		...
		LOG_INF("[S5K5E2YA_OFILM_MIPI_read_otp_wb] flag is error!!");
		//if((flag1!=0x01)&&(flag2!=0x01)&&(flag3!=0x01))
		//{
			LOG_INF("[S5K5E2YA_OFILM] [S5K5E2YA_OFILM_MIPI_read_otp_wb] otp AWB ERROR!");
			otp->R_to_G = OFILM_tRG_Ratio_typical;
			otp->B_to_G = OFILM_tBG_Ratio_typical;
			otp->G_to_G = 0x400;
		//}		
		LOG_INF("[S5K5E2YA_OFILM] [S5K5E2YA_OFILM_MIPI_read_otp_wb] otp->R_to_G=0x%x\n", otp->R_to_G); 
		LOG_INF("[S5K5E2YA_OFILM] [S5K5E2YA_OFILM_MIPI_read_otp_wb] otp->B_to_G=0x%x\n", otp->B_to_G); 
		LOG_INF("[S5K5E2YA_OFILM] [S5K5E2YA_OFILM_MIPI_read_otp_wb] otp->G_to_G=0x%x\n", otp->G_to_G); 
	}
}
void S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1(struct S5K5E2YA_OFILM_MIPI_otp_struct *otp)
{
	...
	otp->R_to_G = (otp->rg_h<<8)|otp->rg_l;
	otp->B_to_G = (otp->bg_h<<8)|otp->bg_l;
	otp->G_to_G = (otp->GbGr_h<<8)|otp->GbGr_l;
	R_to_G = otp->R_to_G;
	B_to_G = otp->B_to_G;
	G_to_G = otp->G_to_G;

	LOG_INF("[S5K5E2Y] [S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1] R_to_G=%x\n", R_to_G);	
	LOG_INF("[S5K5E2Y] [S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1] B_to_G=%x\n", B_to_G);	
	LOG_INF("[S5K5E2Y] [S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1] G_to_G=%x\n", G_to_G);
	...
	LOG_INF("[S5K5E2Y] [S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1] R_gain=0x%x\n", otp->RGain);	
	LOG_INF("[S5K5E2Y] [S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1] B_gain=0x%x\n", otp->BGain);	
	LOG_INF("[S5K5E2Y] [S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1] G_gain=0x%x\n", otp->GGain);
}

void S5K5E2YA_OFILM_MIPI_write_otp_wb(struct S5K5E2YA_OFILM_MIPI_otp_struct *otp)
{
	kal_uint32 r_gain_h,r_gain_l,g_gain_h,g_gain_l,b_gain_h,b_gain_l;
	kal_uint16 R_GainH, B_GainH, G_GainH;
	kal_uint16 R_GainL, B_GainL, G_GainL;
	kal_uint32 temp;
	LOG_INF("S5K5E2YA_OFILM_MIPI_write_otp_wb enter!!");
	temp = otp->RGain;
	...
	r_gain_h = read_cmos_sensor(0x0210);
	r_gain_l = read_cmos_sensor(0x0211);
	g_gain_h = read_cmos_sensor(0x020e);
	g_gain_l = read_cmos_sensor(0x020f);
	b_gain_h = read_cmos_sensor(0x0212);
	b_gain_l = read_cmos_sensor(0x0213);
	LOG_INF("[S5K5E2YA_OFILM_MIPI_write_otp_wb]0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",r_gain_h,r_gain_l,
		g_gain_h,g_gain_l,b_gain_h,b_gain_l);	
}
void S5K5E2YA_OFILM_MIPI_update_wb_register_from_otp(void)
{
	struct S5K5E2YA_OFILM_MIPI_otp_struct current_otp;
	LOG_INF("[S5K5E2Y_MIPI_update_wb_register_from_otp] enter!!\n");
	...
	LOG_INF("[S5K5E2Y_MIPI_update_wb_register_from_otp] exit!!\n");
}
#endif
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x0305,0x06); //PLLP (def:5)
	write_cmos_sensor(0x0306,0x00);
	write_cmos_sensor(0x0307,0xC7); //PLLM (def:CCh 204d --> B3h 179d)
	write_cmos_sensor(0x3C1F,0x00); //PLLS
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		LOG_INF("Read111 sensor id fail,i2c_write_id 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
	...
	otp_read_old_id();
	S5K5E2YA_OFILM_MIPI_read_otp_wb(&current_otp);
	LOG_INF("Read module_id_old=0x%x,module_id_new=0x%x\n",module_id_old,module_id_new);

	if((0x02 != module_id_new) && (0x02 != module_id_old)){
	    *sensor_id = 0xFFFFFFFF;
	    return ERROR_SENSOR_CONNECT_FAIL;
	}
#endif
...
#ifdef S5K5E2YA_OTP_FUNCTION
	otp_read_old_id();
	LOG_INF("module_id_old=0x%x,module_id_new=0x%x\n",module_id_old,module_id_new);
	if(0x02 == module_id_new){
		LOG_INF("module_id_new = 0x%x\n",module_id_new);
/*---------quznzheng.wt, delete module_id_old  -------------------*/
